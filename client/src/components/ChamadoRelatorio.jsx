...
export default function ChamadoRelatorio() {
  ...
  const load = useCallback(async (next) => { setLoading(true); setError(""); try { setResult(await getChamadoRelatorio(toPayload(next))); } catch(e) { setError(e.message || "Não foi possível carregar o relatório."); } finally { setLoading(false); } }, []);
  ...
  return <div className="chamado-report">
    <header className="chamado-report-heading"><div><span className="workspace-label">Gestão operacional</span><h2>Relatórios de chamados</h2><p>Filtre a operação e exporte os dados em CSV ou Excel.</p></div><div><button className="button-standard" onClick={() => void exportFile("csv")} disabled={!!exporting}><Download size={17}/> {exporting==="csv"?"Gerando...":"CSV"}</button><button className="button-standard" onClick={() => void exportFile("xlsx")} disabled={!!exporting}><FileSpreadsheet size={17}/> {exporting==="xlsx"?"Gerando...":"Excel"}</button></div></header>
    {error && <FeedbackMessage type="error" compact>{error}</FeedbackMessage>}
    <form className="chamado-report-filters" onSubmit={apply}>
      <label>Período inicial<input type="date" name="criadoDe" value={filter.criadoDe} onChange={change}/></label><label>Período final<input type="date" name="criadoAte" value={filter.criadoAte} onChange={change}/></label>
    ...
    <section className="chamado-report-table"><div className="report-total"><strong>{result.total}</strong> chamados encontrados</div>{loading ? <LoadingState message="Carregando relatório..." /> : <div className="report-table-scroll"><table><thead><tr><th>Número</th><th>Chamado</th><th>Status</th><th>SLA</th><th>Prioridade</th><th>Categoria</th><th>Atendente</th><th>Abertura</th><th>1ª resposta</th><th>Resolução</th></tr></thead><tbody>{result.items.map((item)=><tr key={item.id}><td>#{item.numero}</td><td><strong>{item.titulo}</strong><small>{item.solicitante}</small></td><td>{label(item.status)}</td><td><span className={`report-sla sla-${item.slaStatus.toLowerCase()}`}>{label(item.slaStatus)}</span></td><td>{item.prioridade}</td><td>{item.categoria}</td><td>{item.atendente}</td><td>{date(item.criadoEm)}</td><td>{minutes(item.tempoPrimeiraRespostaMinutos)}</td><td>{minutes(item.tempoResolucaoMinutos)}</td></tr>)}{!result.items.length&&<tr><td colSpan="10" className="report-empty">Nenhum chamado encontrado.</td></tr>}</tbody></table></div>}
      <footer className="report-pagination"><button type="button" className="button-standard" disabled={result.page<=1||loading} onClick={()=>go(result.page-1)}>Anterior</button><span className="report-page-status" aria-live="polite">Página {result.page} de {result.totalPages}</span><button type="button" className="button-standard" disabled={result.page>=result.totalPages||loading} onClick={()=>go(result.page+1)}>Próxima</button></footer>
    </section>
  </div>;
}
