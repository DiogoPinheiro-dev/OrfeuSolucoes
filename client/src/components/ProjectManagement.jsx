...
const TRANSICOES = {
    ...
    EM_ORCAMENTO: ["EM_ORCAMENTO", "CANCELADO"],
    ...
    CANCELADO: ["CANCELADO", "EM_ORCAMENTO"]
};
...
export default function ProjectManagement({ permissions }) {
    ...
    const [pageInfo, setPageInfo] = useState({ total: 0, pagina: 1, limite: 5, totalPaginas: 0 });
    ...
    const loadEditorData = async (project = null) => {
        setEditorError("");
        try {
            const fullProject = project ? await getProjeto(project.id) : null;
    ...
    return <div className="project-management">
        ...
        <CrudGrid title="Cadastro de projetos" kicker="Projetos" columns={columns} rows={rows} selectedId={selectedId} onSelect={setSelectedId} onCreate={() => loadEditorData()} onEdit={loadEditorData} onView={openDetail} toolbarActions={<>
            ...
            {selected?.permissoes?.podeReativar && selected.arquivadoEm && <button className="project-toolbar-action" type="button" onClick={() => reactivateSelected(selected)}>Reativar</button>}
        </>} search={search} onSearchChange={(value) => { setSearch(value); setPageInfo((current) => ({ ...current, pagina: 1 })); }} filters={<><label>Metodologia<select value={filters.metodologia} onChange={(event) => changeFilter("metodologia", event.target.value)}><option value="">Todas</option>{Object.entries(METODOLOGIAS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Status<select value={filters.situacao} onChange={(event) => changeFilter("situacao", event.target.value)}><option value="">Todos</option>{Object.entries(SITUACOES).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Saúde<select value={filters.saude} onChange={(event) => changeFilter("saude", event.target.value)}><option value="">Todas</option>{Object.entries(SAUDES).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="project-archive-filter"><input type="checkbox" checked={filters.incluirArquivados} onChange={(event) => changeFilter("incluirArquivados", event.target.checked)} />Incluir arquivados</label></>} paginationConfig={{ mode: "server", page: pageInfo.pagina, pageSize: pageInfo.limite, totalItems: pageInfo.total, onPageChange: (page) => setPageInfo((current) => ({ ...current, pagina: page })) }} paginationResetKey={`${filters.metodologia}|${filters.situacao}|${filters.saude}|${filters.incluirArquivados}`} emptyMessage={filters.incluirArquivados ? "Nenhum projeto encontrado, incluindo os arquivados." : "Nenhum projeto ativo encontrado."} busy={loading} canCreate={permissions?.podeIncluir === true} canEdit={!!selected && selected.permissoes?.podeAlterar && !selected.arquivadoEm} canView={selected?.permissoes?.podeVisualizar !== false} showDelete={false} selectable={false} />
