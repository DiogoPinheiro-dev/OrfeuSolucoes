  ...
  return <div className="email-solution-management">
    ...
    <section className="workspace-panel workspace-panel-wide">
      ...
      {(canCreate || (editingAccount && canEdit)) && <form className="chamado-email-form" onSubmit={submitAccount}>
        ...
        <button disabled={busy}>{editingAccount ? "Salvar conta" : "Cadastrar conta"}</button>{editingAccount && <button type="button" onClick={() => { setEditingAccount(null); setAccountForm(emptyAccount); }}>Cancelar</button>}
      </form>}
      <div className="chamado-email-list">{accounts.map((item) => <article key={item.id}><div><strong>{item.nome}</strong><span>{item.emailGoogle} · {item.tipo === "GMAIL" ? "Gmail pessoal" : "Workspace"} · {item.conectado ? "Conectada" : "Aguardando autorizacao"}</span></div><div>{canEdit && <><button onClick={() => editAccount(item)}>Editar</button><button onClick={() => connect(item.id)}>{item.conectado ? "Reconectar" : "Conectar Google"}</button></>}{canDelete && <button onClick={async () => { if (confirm("Desativar esta conta?")) { await deleteGoogleEmailConta(item.id); await load(); } }}>Desativar</button>}</div></article>)}</div>
    </section>
    <section className="workspace-panel workspace-panel-wide">
      ...
      {(canCreate || (editingSender && canEdit)) && <form className="chamado-email-form" onSubmit={submitSender}>
        ...
        <label><input name="ativo" type="checkbox" checked={senderForm.ativo} onChange={changeSender}/> Ativo</label><button disabled={busy}>{editingSender ? "Salvar remetente" : "Cadastrar remetente"}</button>{editingSender && <button type="button" onClick={() => { setEditingSender(null); setSenderForm(emptySender); setAliases([]); }}>Cancelar</button>}
      </form>}
      <div className="chamado-email-list">{senders.map((item) => <article key={item.id}><div><strong>{item.solucaoNome}</strong><span>{item.remetenteNome ? `${item.remetenteNome} · ` : ""}{item.remetenteEmail} · conta {item.googleContaNome} · {item.ativo ? "Ativo" : "Inativo"}</span></div><div>{canEdit && <button onClick={() => void editSender(item)}>Editar</button>}{canDelete && <button onClick={async () => { if (confirm("Desativar este remetente?")) { await deleteChamadoSolucaoEmail(item.id); await load(); } }}>Desativar</button>}</div></article>)}</div>
    </section>
  </div>;
}