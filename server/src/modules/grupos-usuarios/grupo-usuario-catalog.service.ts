...
export class GrupoUsuarioCatalogService {
  ...
  async create(input: CreateGrupoUsuarioInput): Promise<GrupoUsuarioType> {
    ...
    const created = await this.prisma.$transaction(async (tx) => {
      const record = (await (tx as never as { grupoUsuario: { create: Function } }).grupoUsuario.create({
        data: {
          nome,
  ...
  async update(input: UpdateGrupoUsuarioInput): Promise<GrupoUsuarioType> {
    ...
    const shouldSyncAccess = input.solucaoIds !== undefined
      || input.funcionalidadeIds !== undefined
      || input.funcionalidadePermissoes !== undefined;
    const funcionalidadeIds = input.funcionalidadeIds
      ?? input.funcionalidadePermissoes?.map((permissao) => permissao.funcionalidadeId)
      ?? [];
    const updated = await this.prisma.$transaction(async (tx) => {
      const record = (await (tx as never as { grupoUsuario: { update: Function } }).grupoUsuario.update({
        where: { id: input.id },
        data: {
      ...
      if (shouldSyncAccess) {
        await this.solucoesService.syncGroupAccess(
          ...
          input.solucaoIds ?? [],
          funcionalidadeIds,
          this.grupoUsuarioPermissao.resolveFuncionalidadePermissoes(
            funcionalidadeIds,
            input.funcionalidadePermissoes,
            { ...current, ...input }
          ),
          tx
        );
  ...
  async remove(id: number): Promise<boolean> {
    ...
    await this.prisma.$transaction(async (tx) => {
      await (tx as never as { usuario: { updateMany: Function } }).usuario.updateMany({
        where: { grupoId: id },
        data: { grupoId: null, sessaoVersao: { increment: 1 } }
      });
      await (tx as never as { grupoUsuario: { delete: Function } }).grupoUsuario.delete({ where: { id } });
    });
